...
object S7Service : BaseAppService(AppConfigs.LAYLA) {
    ...
    override fun extractChatMessage(
        ...
        val hasUserTag = root.findAccessibilityNodeInfosByViewId("com.layla.chat:id/iv_head")
            ?.any { tagNode ->
    ...
    override suspend fun handleChat(
        ...
        try {
            ...
            repeat(3) { attempt ->
                ...
                isInsideChat = checkRoot?.findAccessibilityNodeInfosByViewId("com.layla.chat:id/tv_nickname")
            ...
            if (input == null) {
                ...
                val isStillInChat = root2?.findAccessibilityNodeInfosByViewId("com.layla.chat:id/voiceCallGift")
                    ?.any { !WindowFilterHelper.isOwnOverlay(it) } ?: false
    ...
    private suspend fun performBackOnce(service: AccessibilityService) {
        try {
            ...
            if (stillInChat) {
                ...
                // Final check
                val checkRoot = WindowFilterHelper.getTargetRootNode(service, config.packageName)
                val finalCheck = checkRoot?.findAccessibilityNodeInfosByViewId("com.layla.chat:id/voiceCallGift")
                    ?.any { !WindowFilterHelper.isOwnOverlay(it) } ?: false
    ...
    private suspend fun extractOpenedChatNameAsync(
        ...
        while (System.currentTimeMillis() - startTime < timeoutMs) {
            ...
            val name = withContext(Dispatchers.IO) {
                ...
                val titleNode = root.findAccessibilityNodeInfosByViewId("com.layla.chat:id/tv_nickname")
                    ?.firstOrNull {
                        !WindowFilterHelper.isOwnOverlay(it) &&
    ...
    private suspend fun waitForLastMessageAsync(
        ...
        while (System.currentTimeMillis() - start < timeoutMs) {
            ...
            val message = withContext(Dispatchers.IO) {
                ...
                val lastBubble = root.findAccessibilityNodeInfosByViewId("com.layla.chat:id/tv_content")
                    ?.filter { !WindowFilterHelper.isOwnOverlay(it) }
                    ?.lastOrNull()
                    ?: return@withContext null

                // Priority 1: Reply content
                val replyMsg = lastBubble.findAccessibilityNodeInfosByViewId("com.layla.chat:id/tv_content")
